...
export const Label = styled.span`
	font-family: Arial, sans-serif;
	color: ${(props) =>
		props.colour ? `${toHexColourString(props.colour)}` : "black"};
`;
...
export const Line = styled.div`
	...
	background-color: ${(props) =>
		props.colour
			? toHexColourString(getComplementaryColour(props.colour))
			: "white"};
	font-size: 1.5em;
	font-family: Arial, sans-serif;
	padding: 0.5em;
...
function toHexColourString(colour) {
	...
	hexColourString = `#${"000000".substr(
		0,
		6 - hexColourString.length
	)}${hexColourString}`;
